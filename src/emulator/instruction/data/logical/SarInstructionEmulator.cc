#include "SarInstructionEmulator.h"

#include "../../../../execution/ExecutionThread.h"

bool SarInstructionEmulator::EmulateInstruction(
  ...
  const int64_t signed_dst = static_cast<int64_t>(dst);
  const uint64_t result = static_cast<uint64_t>(signed_dst >> count);

  WriteOperand(thread->GetGuestRegisters(), operands[0], result);

  if (result > 0) {
    UpdateFlags(thread->GetGuestRegisters(), dst, count, result,
                operands[0].size, kSar);
  }
